...
public class ProductSearchImpl extends QuerydslRepositorySupport implements ProductSearch{
  ...
  public PageResponseDTO<ProductListDTO> list(PageRequestDTO pageRequestDTO) {
    ...
    query.leftJoin(product.images, productImage); // 참조가 없는 관계에서도 outer join 가능하게
  ...
  public PageResponseDTO<ProductListDTO> listWithReview(PageRequestDTO pageRequestDTO) {
    ...
    JPQLQuery<Product> query = from(product);
    query.leftJoin(product.images, productImage); // 참조가 없는 관계에서도 outer join 가능하게
    query.leftJoin(review).on(review.product.eq(product)); // 정상적인 outer join 조건
    ...
    JPQLQuery<ProductListDTO> dtoQuery =
      query.select(
        Projections.bean(ProductListDTO.class, 
        product.pno, product.pname, product.price, productImage.fname.min().as("fname"), review.score.avg().as("reviewAvg"), review.count().as("reviewCnt"))
      );
